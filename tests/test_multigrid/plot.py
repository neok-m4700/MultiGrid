import numpy as np
import pylab as plt; plt.style.use('thesis')
...
n = 1 + 2**(j + 1)
ans = np.fromfile('answer.dat'.format(j), dtype=np.float64).reshape(n, n)
...
imgs = [None] * 4
imgs[0] = axes[0, 0].pcolorfast(ans.T)
imgs[1] = axes[0, 1].pcolorfast(src.T)
imgs[2] = axes[1, 0].pcolorfast(res.T)
imgs[3] = axes[1, 1].pcolorfast(np.log10(err + 1e-8).T)
...
plt.savefig(f'plot_{suffix}.pdf')